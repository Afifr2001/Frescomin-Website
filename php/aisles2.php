<?php
	...
	$products = file_get_contents("../json/products.json");//gets the contents from json file
	$array = json_decode($products, true);//convert int oan array
	$length = count($array);//length of array
	$list = "";//
	for($i = 0; $i < $length; $i++){
		if($array[$i]["category"] === "FandV"){
			$list .="	<li>
